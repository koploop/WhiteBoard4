...
## RDV0.1.0
- 添加一种状态：称之为多指状态（即每个触摸点是不同的状态）：
  - 和ZoomPanState互斥，关闭画布缩放，且工具处于书写工具下，根据多指触摸点大小执行对应操作
  - 有多个触摸点时，根据每个触摸点的面积（touchMajor）判断触摸点类型：书写笔、手指触摸移动、或者橡皮擦。
  - 计算方法使用HikDefaultTouchCalc类的getTouchType方法
  - 类型是笔时即书写，类型是手指触摸时即移动元素操作（这种状态下不再进行圈选线条绘制，直接移动第一个触摸点命中的元素），类型是橡皮擦时即擦除元素操作。
  - 多个操作可以同时进行不互斥
