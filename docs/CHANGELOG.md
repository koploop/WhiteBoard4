...
### 新增
- 实现 RDV0.2.0 便签元素能力：
    - 新增 `NoteElement`，支持背景色、文本内容、内部批注线条数据。
    - 便签支持选中能力：移动、缩放、改色、文字编辑、批注、批注擦除、删除、复制、层级调整。
    - 在“更多工具”面板接入“便签”入口，点击后可直接插入便签并自动选中。
- 新增通用原生 View 承载层：
    - 在 `boardimpl` 中新增 `NativeViewHostLayer`，通过适配器机制管理原生 View 生命周期。
    - 首个适配器接入 `EditText` 便签视图，支持后续扩展 WebView、自定义 View 等原生元素。

### 变更
- 扩展选中菜单动作：
    - 新增 `EDIT_TEXT`、`ANNOTATE`、`ERASE_ANNOTATION` 能力入口。
    - 支持从选中菜单切换便签文字编辑、批注、批注擦除模式。
- 扩展白板状态：
    - `WhiteBoardState` 新增 `activeNoteId` 与 `noteInteractionMode`，用于管理当前便签交互上下文。
- 扩展 SDK 接口：
    - 增加便签文本更新、批注开始/追加/结束、批注擦除等原子方法。
    - 在 DRAW 态下支持便签批注与批注擦除事件分流。

### 修复
- 修复原生 View 类元素选中行为：
    - 在 `TouchHandler` 增加原生 View 元素命中拦截，书写/擦除/选中任意模式下点击原生 View 元素都会进入选中态。
- 修复便签编辑态退出问题：
    - 便签处于文字编辑态时，点击空白区域会清理 `noteInteractionMode`，并触发原生 `EditText` 失焦，光标与编辑态同步退出。
- 修复便签移动时文字不同步问题：
    - 为 `WhiteBoardState` 增加 `uiVersion`，并在元素更新链路递增，确保 `StateFlow` 在元素原地变换时也会分发状态，驱动原生视图层同步位置。
- 优化便签移动流畅度：
    - 在 `MainActivity` 新增原生视图逐帧同步机制（Choreographer），按帧读取最新白板状态并同步 `NativeViewHostLayer`，减少拖动过程中的视觉延迟。
- 修复逐帧同步引入的编辑态回退问题：
    - 编辑态下不再将元素文本强制回写到 `EditText`，避免输入过程被每帧覆盖。
    - 键盘显示/收起改为基于编辑态切换沿触发（进入沿/退出沿），避免逐帧重复抢焦点导致无法退出。
    - 新增独立 `id` 作为 `EditText` 标记位，区分“文本上推开关”和“编辑态上次值”。

## [0.1.6] - 2026-03-10

### 新增
- 实现 RDV0.1.1 图片元素能力：
    - 新增 `ImageElement` 元素类型，支持移动、缩放、旋转、改变层级四种选中能力。
    - 在“更多工具”面板中接入“插入图片”点击事件，支持从系统选择器选取本地图片/相册图片并插入白板。
    - 插入图片后自动根据当前画布缩放与视口尺寸计算初始展示尺寸，并居中放置到可视区域中。

### 变更
- 升级选中变换回调机制：
    - 在 `BaseElement.OnSelectedTransformListener` 中新增矩阵实时回调 `onMatrixChanged(matrix)`。
    - 在移动、缩放、旋转过程中，通过 SDK 统一实时分发位移/缩放/旋转与最新矩阵回调，满足元素内部扩展需求。
- 优化“更多工具”面板交互：
    - 新增工具点击监听入口，点击工具项后自动关闭面板并分发具体工具事件。
- 优化粘贴逻辑：
    - `pasteElements()` 改为基于 `BaseElement.copy()` 通用复制，支持包括图片元素在内的多类型元素粘贴。
...
### 修复
- 修复图片旋转中心异常问题：
    - 重构 `ImageElement` 坐标系逻辑，将 `(x, y)` 定义为图片中心点而非左上角，确保旋转操作围绕图片中心进行。
    - 修正 `BaseElement.getTransformMatrix()` 矩阵变换顺序，使其与 Canvas 绘制顺序一致（先缩放、再旋转、最后平移），解决命中检测与绘制不一致的问题。
