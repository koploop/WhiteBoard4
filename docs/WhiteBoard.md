...
- **NoteElement**: 便签元素，包含背景色、文本内容、批注线条集合；支持选中后移动、缩放、改色、文字编辑、批注与批注擦除。
...
- **NativeViewHostLayer**: 通用原生 View 承载层，统一管理元素ID与原生View实例映射，处理创建/更新/销毁与世界坐标到屏幕坐标同步，可扩展承载 `EditText`、`WebView`、自定义 View。
...
### V0.2.0 - 便签元素与通用原生 View 承载层
- **便签元素 (NoteElement)**:
    - 新增便签元素，支持背景色、文本内容与内部批注线条。
    - 选中能力新增：文字编辑、批注、批注擦除。
    - 便签内部批注线条由元素内聚管理，擦除仅作用于批注，不影响文本。
- **通用原生 View 承载层**:
    - 在 `boardimpl` 中新增 `NativeViewHostLayer`，通过适配器机制承载原生 View 元素。
    - 承载层统一处理元素变换到 View 位置/旋转/尺寸同步，避免耦合到具体元素实现。
    - 便签作为首个接入适配器；后续可扩展接入 WebView 与任意自定义 View。
- **便签交互模式**:
    - 在 `WhiteBoardState` 中新增 `activeNoteId` 与 `noteInteractionMode`（`NONE/TEXT_EDIT/ANNOTATE/ERASE`）。
    - 选中菜单动作可切换便签文字编辑、批注、批注擦除，并与现有状态机协同工作。
